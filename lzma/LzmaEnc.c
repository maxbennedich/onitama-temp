...
SRes LzmaEncodeToOutStream(ISeqOutStream *outStream, const Byte *src, SizeT srcLen,
    const CLzmaEncProps *props, ICompressProgress *progress, ISzAllocPtr alloc, ISzAllocPtr allocBig)
{
  CLzmaEnc *p = (CLzmaEnc *)LzmaEnc_Create(alloc);
  SRes res;
  if (!p)
  ...
  if (res == SZ_OK)
  {
    ...
    res = LzmaEnc_WriteProperties(p, header, &headerSize);
    for (int i = 0; i < 8; i++)
    ...
    if (outStream->Write(outStream, header, headerSize) != headerSize) {
      ...
      if (res == SZ_OK) {
        CLzmaEnc *pe = (CLzmaEnc *)p;

        pe->writeEndMark = 0;
        pe->rc.outStream = outStream;
